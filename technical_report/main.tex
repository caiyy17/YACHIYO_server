...
Not everything is a signal. When a message's meaning depends on its position in the data flow (\texttt{SoS} before its stream, \texttt{EoS} after the last chunk), it must ride in band as above. But some facts are \emph{session-level}: they reference the past by timestamp or identity alone, and in-band ordering buys them nothing while costing latency and per-hop declarations. These are \emph{events}, delivered out of band on the control plane. Cancellation (Section~\ref{sec:cancel}) is the built-in archetype---its semantics are deliberately order-free (invalidate everything older than $T$). Beyond the built-in verbs (\texttt{cancel}, \texttt{kill}), a pipeline declares its event vocabulary in config, mirroring the signal surface: a top-level \texttt{events} list names the verbs the entry routes to the control plane (also exempting them from the monotonicity check, since events reference the past), and each consuming node declares \texttt{catch\_events} entries---the same explicit one-to-one \texttt{\{source, target\}} format as \texttt{catch\_signals}, renames and null opt-outs included---matched exactly against its class contract \texttt{REQUIRED\_CATCH\_EVENTS}. Delivery is broadcast: the per-pipeline event handler copies the verb into every node's control queue (excluding the emitting node); a node that declared it consumes it through a dedicated hook, every other node skips it silently, and events never touch cancellation state. One pipeline-level check closes the loop: the \texttt{events} list must equal the union of the nodes' wired \texttt{catch\_events} sources, exactly both ways---every declared verb has a catcher, no node catches an undeclared verb. Current event verbs: \texttt{connection\_start} (transport ready; starts clock-driven output) and \texttt{playback\_complete} (the client's playback acknowledgment: it paces batched-input release, and---carrying the \texttt{response\_id}/\texttt{item\_id} of the item that was playing when playback stopped---lets the LLM node truncate the persisted turn to the prefix that was actually heard).
...
In WebSocket mode, output is organized per sentence: a message carries a complete TTS segment ($\sim$0.5--1\,s of audio) or, in the streaming configurations, one fixed 200\,ms chunk inside the sentence's envelope. Either way the client receives self-contained clips and paces playback itself. WebRTC mode re-granularizes the output to frame-level:
\begin{itemize}[nosep]
...
\begin{table}[h]
    ...
    \begin{tabular}{lll}
        ...
        \midrule
        Message granularity & Sentence or 200\,ms chunk & Frame group (100\,ms) \\
...
\begin{figure}[h]
    \centering
    \fbox{\parbox{0.9\linewidth}{\centering\vspace{2em}\textit{[System architecture. Unity client $\leftrightarrow$ WebSocket/WebRTC $\leftrightarrow$ FastAPI server $\leftrightarrow$ Standalone services (ASR, LLM, TTS, Motion, VAD, VectorDB)]}\vspace{2em}}}
...
Each user gets an exclusive pipeline instance (threads, queues, conversation state). Compute-heavy models are deployed as standalone services:
\begin{itemize}[nosep]
    \item ASR: Qwen3-ASR server (GPU, vLLM backend)
    \item LLM: vLLM server (GPU)
    \item TTS: Qwen3-TTS server (GPU, faster-qwen3-tts)
    \item Motion: Flood streaming text-to-motion server (GPU, WebSocket)
    \item VAD: streaming voice-activity service (silero)
    \item Vector DB: BGE embedding + FAISS server (GPU/CPU)
\end{itemize}

Pipeline nodes are thin service clients: they create an OpenAI SDK client (or the service's small custom client), call the API, and process the response. Multiple users' pipelines share the same model servers concurrently. The model services expose OpenAI-compatible endpoints, so swapping a local model for a cloud API requires only a configuration change; services with no official API shape (the streaming VAD, the Flood motion WebSocket, the vector query) use small custom protocols behind the same settings mechanism.
...
The client is built with Unity 6 (C\#) using WebSocket-Sharp and Unity WebRTC for communication, and the Animator controller with blend shapes for character animation. The system comprises $\sim$25 pipeline module types, dynamically discovered via a module registry. All configuration is JSON-based: pipeline configs, dataset configs, and model configs. Deployment uses per-service conda environments with a startup script that launches services sequentially and verifies port readiness.
...
\begin{table}[h]
    \centering
    \small
    \caption{Representative pipeline configurations.}
    \label{tab:configs}
    \begin{tabular}{@{}p{0.24\linewidth}p{0.70\linewidth}@{}}
        \toprule
        \textbf{Config} & \textbf{Pipeline} \\
        \midrule
        Conversation & ASR $\rightarrow$ LLM $\rightarrow$ DataQuery $\rightarrow$ Dispatch $\rightarrow$ (Motion RAG $\|$ TTS) $\rightarrow$ Receive $\rightarrow$ Pad \\
        Streaming humanoid & ServerVAD $\rightarrow$ ASR $\rightarrow$ LLM $\rightarrow$ DataQuery $\rightarrow$ StreamTTS $\rightarrow$ MotionChunkGeneration \\
        VTuber livestream & DanmakuBuffer $\rightarrow$ LLM $\rightarrow$ DataQuery $\rightarrow$ StreamTTS $\rightarrow$ MotionChunkGeneration \\
        WebRTC video stream & FrameCollector $\rightarrow$ ServerVAD $\rightarrow$ ASR $\rightarrow$ LLM $\rightarrow$ DataQuery $\times 2$ $\rightarrow$ StreamTTS $\rightarrow$ VideoChunkGeneration $\rightarrow$ FrameSplitter \\
        \bottomrule
    \end{tabular}
\end{table}
    ...
    },
    {
      ...
      "config": {
        ...
        "output_vars": [{"source": "text",
                        ...
                        {"source": "item_id",
                         "target": null}],
        "catch_events": [{"source": null,
                          "target": "playback_complete"}],
...
The model services expose OpenAI-compatible endpoints wherever an official API shape exists:
\begin{itemize}[nosep]
