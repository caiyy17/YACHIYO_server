...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Cancellation consistency.} After a cancellation event $\text{cancel}(T)$ is issued, no message with timestamp $< T$ is subsequently delivered to the client.
\end{enumerate}
...
Each node has a dedicated \texttt{cancel\_queue} (parallel to its data queue), checked before each dequeue operation. When the client sends $\texttt{cancel}(T)$, the server broadcasts $T$ to every node's cancel queue. Each node updates its local \texttt{cancel\_timestamp}; any subsequently dequeued message with timestamp $<$ \texttt{cancel\_timestamp} is silently dropped.

The inequality is deliberately \emph{strict}, uniformly across server and client; whether the boundary is included is expressed by the issuer's choice of $T$ (nudged by an epsilon where needed---e.g.\ a barge-in cancel spares the concurrent same-instant utterance, while disconnect cleanup includes the final turn).

The send queue performs a final filter: before transmitting any message to the client, it verifies that the message's timestamp is not older than the current cancel timestamp. This provides a safety net for messages that were already past intermediate nodes when the cancel arrived.
...
Adapter nodes bridge the paradigms. The frame $\rightarrow$ event side is a pair: \textbf{FrameCollector} transforms each arriving frame group into per-lane pipeline messages (the group's audio frames joined into one WAV chunk, video frames and data slots passed as lists), and \textbf{VAD} keeps a persistent ring of those audio chunks, cutting an utterance from it between \texttt{recording\_start} and \texttt{recording\_end} (with configurable pre/post-roll). The event $\rightarrow$ frame side is \textbf{FrameSplitter}, a clock-driven output module that splits TTS audio into 20\,ms PCM frames paired with video and data. It overrides the standard message-driven processing loop with an absolute-time tick loop, pausing until a \texttt{connection\_start} signal arrives and then outputting one synchronized group per tick regardless of whether speech data is available (filling with silence and idle frames when idle). The core pipeline remains purely event-driven.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Cancellation}: checks a thread-safe \texttt{cancel\_timestamp} before processing each message; messages with timestamp $<$ \texttt{cancel\_timestamp} are silently dropped (Section~\ref{sec:cancel}).
...
Some modules process data spanning multiple messages (e.g., the VAD accumulates audio in its ring and cuts a segment between \texttt{recording\_start} and \texttt{recording\_end}). \texttt{SpanProcessingStep} extends \texttt{BaseProcessingStep} with: \texttt{start\_span(timestamp)} to begin accumulation, \texttt{end\_span()} to complete it, and \texttt{on\_span\_cancel()} to discard accumulated state on cancellation.
