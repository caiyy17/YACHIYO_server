...
A pipeline is a JSON configuration listing processing nodes in topological order. Each node has a unique, monotonically increasing ID, a processing function (resolved from a module registry), and a configuration block specifying its data routing (detailed in Section~\ref{sec:routing}) and downstream connections. Variable names are prefixed with the producing node's ID (e.g., \texttt{3\_text}) to avoid namespace collisions. Node IDs need not be contiguous, allowing insertion without renumbering. See Appendix~\ref{app:config} for a complete example.

\subsection{Why Linearization}
...
\begin{figure}[h]
    \centering
    \fbox{\parbox{0.9\linewidth}{\centering\vspace{2em}\textit{[Figure placeholder: DAG linearization diagram. Left: DAG with skip-connections. Right: linearized queue chain $Q_0 \rightarrow N_0 \rightarrow Q_1 \rightarrow N_1 \rightarrow \cdots \rightarrow Q_n \rightarrow \text{send\_queue}$, with destination-based routing shown]}\vspace{2em}}}
...
\begin{figure}[h]
    \centering
    \fbox{\parbox{0.9\linewidth}{\centering\vspace{2em}\textit{[Figure placeholder: Streaming timeline. Horizontal axis = time. Rows: LLM (streaming sentences $s_1, s_2, s_3, \ldots$), TTS (synthesizing $s_1$ while LLM produces $s_2$), Client (playing $s_1$ audio while TTS synthesizes $s_2$). Show overlap between stages.]}\vspace{2em}}}
...
L_{\text{stream}}^{\text{first}} = L_{\text{ASR}} + L^{\text{LLM}}_1 + L_{\text{mid}} + L^{\text{TTS}}_1 + L_{\text{net}}
\end{equation}
where $L_{\text{mid}}$ is the total processing time of intermediate nodes between LLM and TTS. When $L_{\text{mid}}$ is small (e.g., a lightweight RAG lookup), it can be omitted from the approximations below; when it is large (e.g., motion generation), the dispatcher--receiver bracket (Section~\ref{sec:parallel}) can parallelize it with TTS.
...
\begin{equation}
\label{eq:speedup}
S = \frac{L_{\text{batch}}}{L_{\text{stream}}^{\text{first}}} = \frac{L_{\text{ASR}} + \sum_k L^{\text{LLM}}_k + \sum_k L^{\text{TTS}}_k + L_{\text{net}}}{L_{\text{ASR}} + L^{\text{LLM}}_1 + L^{\text{TTS}}_1 + L_{\text{net}}}
\end{equation}

\subsection{Data Routing and Variable Mapping}
\label{sec:routing}

Each inter-node message is a flat key-value object. Variable names are prefixed with the producing node's ID to avoid collisions. Three routing mechanisms handle data flow without requiring nodes to know the full pipeline topology: (1) \emph{input extraction} selects which upstream fields a node consumes, with ordered fallback sources; (2) \emph{output mapping} maps processing results to prefixed target names; (3) \emph{pass-through forwarding} carries metadata unchanged across nodes that do not consume it (e.g., the LLM's action tag passes through TTS to reach the client). Pass-through is only applied when a node actually processes a message; forwarded messages (destination mismatch) are passed unchanged. A \emph{destination} field enables skip-connections to non-adjacent nodes. See Appendix~\ref{app:config} for a concrete configuration example.

\subsection{Signal Propagation: An Analogy with CPU Pipelines}
\label{sec:signals}
...
\subsection{Cancellation Consistency}
\label{sec:cancel}
...
% ============================================================
\section{Client-Server Synchronization}
\label{sec:sync}

\subsection{Timestamp Mechanism}
...
WebRTC is a real-time communication protocol that provides \emph{tracks} (continuous media streams for audio and video) and a \emph{DataChannel} (a reliable or unreliable channel for arbitrary application data). The two communication modes in YACHIO reflect fundamentally different timing paradigms:
...
Two adapter nodes bridge the paradigms: AudioCollector assembles continuous microphone frames into a single WAV buffer on \texttt{vad\_end} (frame $\rightarrow$ event); FrameSplitter splits TTS audio into 20\,ms PCM frames (event $\rightarrow$ frame). The core pipeline remains purely event-driven.
...
\subsection{Pipeline Extension for WebRTC}

Two adapter nodes are inserted at the boundaries of the pipeline:
\begin{itemize}[nosep]
    \item \textbf{AudioCollector} (head): collects incoming WebRTC audio frames between \texttt{vad\_start} and \texttt{vad\_end} signals, assembles them into a complete WAV buffer, and pushes it to the ASR node.
    \item \textbf{FrameSplitter} (tail): receives complete TTS audio output (WAV), resamples to WebRTC rate, splits into synchronized frame groups, and pushes them to the WebRTC tracks.
\end{itemize}

The core pipeline (ASR $\rightarrow$ LLM $\rightarrow$ RAG $\rightarrow$ TTS) remains unchanged. AudioCollector and FrameSplitter are \emph{paradigm adapters}---they translate between event-driven and frame-driven data representations at the pipeline boundary, allowing the core pipeline to remain purely event-driven regardless of the transport protocol.

\subsection{Multi-Modal Frame Synchronization}

\begin{figure}[h]
    \centering
    \fbox{\parbox{0.9\linewidth}{\centering\vspace{2em}\textit{[Figure placeholder: Frame group diagram. One 100ms group containing 5 audio frames (20ms each), 3 video frames (33ms each), and 2 data slots (50ms each). Show GCD calculation: GCD(50, 30, 20) = 10 $\rightarrow$ 100ms group period.]}\vspace{2em}}}
...
% ============================================================
\section{Resource Sharing and Scalability}
\label{sec:scaling}
...
\begin{figure}[h]
    \centering
    \fbox{\parbox{0.9\linewidth}{\centering\vspace{2em}\textit{[Figure placeholder: System architecture diagram showing Unity client $\leftrightarrow$ WebSocket/WebRTC $\leftrightarrow$ FastAPI server $\leftrightarrow$ Standalone services (ASR, LLM, TTS, VectorDB)]}\vspace{2em}}}
...
\begin{itemize}[nosep]
    \item ASR: SenseVoice server (GPU)
    \item LLM: vLLM server (GPU)
    \item TTS: BertVITS2 server (GPU)
    \item Vector DB: BGE embedding + FAISS server (GPU/CPU)
\end{itemize}

Pipeline nodes are thin HTTP clients: they create an OpenAI SDK client, call the API, and process the response. Multiple users' pipelines share the same model servers concurrently.
...
\subsection{LLM Node: Tavern-Style Conversation Workflow}
...
All experiments are conducted on a single workstation equipped with an NVIDIA GeForce RTX 5090 GPU (32\,GB VRAM). All local services---SenseVoice (ASR), Qwen3.5-9B-AWQ (LLM via vLLM), BertVITS2 (TTS), BGE-M3 (vector database)---run concurrently on this single GPU. The test input is a 1.6-second Chinese speech clip.

\subsection{Per-Stage and End-to-End Latency}

\begin{table}[h]
    \centering
    \caption{Per-stage and end-to-end latency: local pipeline vs.\ OpenAI API. ``LLM first sentence'' = time to first complete sentence from the StreamCutter. All values in ms (mean $\pm$ std, 3 runs).}
    \label{tab:latency}
    \begin{tabular}{lrr}
        \toprule
        \textbf{Stage} & \textbf{Local (RTX 5090)} & \textbf{OpenAI API} \\
        \midrule
        ASR                     & $33 \pm 1$      & $1179 \pm 265$ \\
        LLM (first sentence)    & $183 \pm 1$     & $933 \pm 183$ \\
        LLM (total generation)  & $339 \pm 17$    & $1262 \pm 279$ \\
        TTS (first sentence)    & $829 \pm 21$    & $1554 \pm 499$ \\
        \midrule
        Server first audio      & $1060 \pm 22$   & $3674 \pm 720$ \\
        E2E first audio (client)& $1101 \pm 32$   & $3725 \pm 724$ \\
        E2E total               & $3609 \pm 53$   & $7084 \pm 1338$ \\
        \bottomrule
    \end{tabular}
\end{table}

The local pipeline achieves a first-audio latency of $\sim$1.1\,s, with TTS synthesis dominating ($\sim$0.8\,s per sentence). The OpenAI API pipeline incurs $\sim$3.7\,s first-audio latency, primarily due to network round-trip times. The streaming architecture means that the total response time ($\sim$3.6\,s local) includes overlapping computation: while TTS synthesizes sentence $n$, the LLM is already generating sentence $n{+}1$.
...
When $U$ concurrent users share services on a single GPU, latency depends on the serialization behavior of each service. The LLM server (vLLM) supports continuous batching and can partially parallelize concurrent requests, but the TTS server (BertVITS2) processes requests sequentially. Modeling the TTS as the serialization bottleneck, the expected first-audio latency for user $u$ (ordered by arrival) is approximately:
\begin{equation}
\label{eq:multiuser}
...
Table~\ref{tab:multiuser} confirms this model at low concurrency: for 1--3 users, first-audio latency increases by $\sim$960\,ms per additional user, consistent with TTS serialization ($\sim$829\,ms plus scheduling overhead). At 5 users the increment grows to $\sim$1.5\,s per user, indicating that services beyond TTS (ASR, LLM) also begin to serialize under higher load. With $R$ replicas of the bottleneck service, the effective capacity scales as $\bar{L} \approx L_{\text{stream}}^{\text{first}} + \frac{U - 1}{2R} \cdot L_{\text{bottleneck}}$.

\subsection{WebRTC Streaming}

In WebRTC mode, the pipeline successfully delivers frame-level synchronized output:
\begin{itemize}[nosep]
    \item 906 DataChannel messages (one 100\,ms frame group each)
    \item 662 non-silent audio frames ($\sim$13.2\,s of speech output)
    \item 3 text segments with synchronized animation actions
    \item Audio, video, and data remain synchronized throughout
\end{itemize}

The FrameSplitter overhead is negligible ($<$1\,ms per group), confirming that frame-level re-granularization does not add meaningful latency.
...
\begin{table}[h]
    ...
    \begin{tabular}{lr}
        ...
        \midrule
        Sequential (MotionGen $\rightarrow$ TTS) & $2123 \pm 2$ \\
        Parallel (MotionGen $\|$ TTS) & $1284 \pm 30$ \\
        \midrule
        Improvement & $839$\,ms ($\downarrow$39.5\%) \\
        \bottomrule
    \end{tabular}
\end{table}

The improvement ($839$\,ms) matches the theoretical prediction: $\sum L_i - \max(L_i) = (884 + 829) - 884 = 829$\,ms, with $\sim$10\,ms dispatcher/receiver overhead.

\subsection{Consumer GPU Deployment (RTX 4090)}

To evaluate deployment on consumer hardware, we replicate the conversation pipeline on a single NVIDIA GeForce RTX 4090 (24\,GB VRAM) using smaller, quantized open-source models: Qwen3-ASR-0.6B (ASR via vLLM), Qwen3.5-4B-AWQ-4bit (LLM via vLLM, with vision encoder enabled), Qwen3-TTS-0.6B via faster-qwen3-tts (TTS), and BGE-M3 (vector database). All four services run concurrently on the single GPU, occupying $\sim$16.5\,GB VRAM total.

\begin{table}[h]
    \centering
    \caption{Per-service VRAM usage on RTX 4090 (24\,GB).}
    \label{tab:vram_4090}
    \begin{tabular}{lr}
        \toprule
        \textbf{Service} & \textbf{VRAM (GB)} \\
        \midrule
        VLLM LLM (Qwen3.5-4B-AWQ-4bit, vision) & 7.0 \\
        ASR (Qwen3-ASR-0.6B, vLLM backend) & 4.6 \\
        TTS (Qwen3-TTS-0.6B, CUDA Graph) & 3.1 \\
        VectorDB (BGE-M3) & 1.7 \\
        \midrule
        Total & 16.5 \\
        \bottomrule
...
\begin{table}[h]
    \centering
    \caption{Per-stage latency on RTX 4090 with smaller models (single run). The test input is the same 1.6-second Chinese speech clip used in Table~\ref{tab:latency}.}
    \label{tab:latency_4090}
    \begin{tabular}{lrr}
        \toprule
        \textbf{Stage} & \textbf{RTX 4090 (4B)} & \textbf{RTX 5090 (9B)} \\
        \midrule
        ASR                     & 92   & $33 \pm 1$ \\
        LLM (total generation)  & 503  & $339 \pm 17$ \\
        TTS (first sentence)    & 1535 & $829 \pm 21$ \\
        \midrule
        Server first audio      & 2006 & $1060 \pm 22$ \\
        E2E total               & 4093 & $3609 \pm 53$ \\
        \bottomrule
    \end{tabular}
\end{table}

The RTX 4090 configuration achieves $\sim$2.0\,s first-audio latency, roughly 2$\times$ that of the RTX 5090 setup. The increase is primarily due to the TTS stage: Qwen3-TTS-0.6B ($\sim$1.5\,s per sentence) is slower than BertVITS2 ($\sim$0.8\,s) on the 5090. ASR latency increases from 33\,ms to 92\,ms due to the smaller GPU's lower memory bandwidth, and LLM generation is slower with the 4B model (503\,ms vs.\ 339\,ms for 9B) partly because the 4B model produces comparable-length outputs with fewer parameters. Despite the increased latency, the pipeline remains within interactive bounds ($<$3\,s first audio) and demonstrates that the architecture scales down to consumer-grade hardware with 24\,GB VRAM.
...
The dispatcher--receiver bracket (Section~\ref{sec:parallel}) supports fork--join parallelism at any point in the pipeline, but requires that parallel branches be independent (no data flow between branches within a bracket). Nested brackets (parallel regions within parallel regions) are possible in principle but have not been tested. The TTS server currently supports only one loaded model at a time, limiting concurrent multi-character scenarios. The WebRTC FrameSplitter assumes fixed frame rates; adaptive rate adjustment would be needed for variable-bandwidth networks.
...
We presented YACHIO, a linearized pipeline architecture for real-time embodied conversational agents. The key contributions are:
\begin{enumerate}[nosep]
...
On a single NVIDIA RTX 5090 GPU hosting all services, the system achieves $\sim$1.1\,s first-audio latency for a fully local pipeline and supports up to 3 concurrent users within interactive latency bounds.
...
\begin{enumerate}
    ...
    \item \textbf{Bandwidth optimization via variable filtering.} To avoid unnecessary data transfer, each node specifies \texttt{input\_vars} (fields it needs) and \texttt{pass\_vars} (fields to forward). Fields not listed in either are discarded. This reduces the message size at each hop without affecting correctness.
\end{enumerate}
...
\section{Pipeline Configuration Example}
\label{app:config}

A complete pipeline configuration for the \texttt{unity\_chan} character:
