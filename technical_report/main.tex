...
Signals travel in the \emph{same} queue as data, hop by hop, exactly like ordinary messages---there is no separate channel and no implicit ``drifting''. What a node does with an arriving signal is declared \emph{entirely in its pipeline config}, mirroring how \texttt{input\_vars}/\texttt{pass\_vars} declare field handling. Two declaration lists hold explicit \texttt{\{source, target\}} entries: both fields are always spelled out, same-name included --- no shorthand, no implicit defaults. Each list is a strict one-to-one map: sources are unique (a signal maps to exactly one name) and targets are unique (many-to-one renames would be an untraceable merge).
...
Pipelines are validated statically at initialization time, before any node is built, so all findings across all nodes are reported at once (HTTP 400) at zero build cost. Every check is a per-module contract: declaration lists well-formed (explicit one-to-one entries); catch targets equal to the module's required catches \emph{exactly} (renamed wiring still satisfies the contract since handlers match target names, while an extra target would be silently swallowed); required inputs declared; emit declarations covering the module's emissions exactly, both directions; plus the dispatcher's structural rules. Validation stays strictly \emph{per-node}: cross-node flow is intentionally not modeled --- that would replicate each module's routing semantics and drift as modules evolve; broken links between nodes surface at runtime through the four-state rules, dropped with a loud per-client log entry at the first node reached.

Environment failures are separated from configuration failures. Node initialization is sequential; each node's initialization runs under a timeout and records its outcome. A failed init (dependent model service down, bad settings entry) \emph{fails fast}: nodes built so far are killed, the client state is reset to re-initializable, and the endpoint returns HTTP 503 with the failing node and cause --- distinct from the validator's 400 (fix the config) in that a 503 is retryable once the service is back.
...
Every node runs the same substrate in a dedicated thread: the queue-driven loop with destination routing, signal handling, variable routing, and cancellation described in Sections~\ref{sec:pipeline}--\ref{sec:cancel}. A module author overrides a single processing method; the substrate handles all pipeline mechanics, so modules need not reason about routing, ordering, or cancellation. A span variant serves modules whose work spans multiple messages (e.g.\ accumulating audio frames between \texttt{recording\_start} and \texttt{recording\_end}), discarding accumulated state on cancellation. Inter-node queues may be bounded, in which case a producer blocks when its downstream queue is full --- natural end-to-end backpressure.
