...
A linearized pipeline processes \emph{data} messages (audio, text, etc.) stage by stage. But some information is not data---it is \emph{control}: ``speech has started'' (\texttt{SoS}), ``speech has ended'' (\texttt{EoS}), ``voice activity detected'' (\texttt{recording\_start}/\texttt{recording\_end}). Control information often needs to reach the client faster than data, because the client must update its state (e.g., begin the listening animation) before any processed data arrives.
...
Signals travel in the \emph{same} queue as data, hop by hop, exactly like ordinary messages---there is no separate channel and no implicit ``drifting''. What a node does with an arriving signal is declared \emph{entirely in its pipeline config}, mirroring how \texttt{input\_vars}/\texttt{pass\_vars} declare field handling. Two declaration lists, both supporting renaming via \texttt{\{source, target\}} entries (by convention written explicitly, defaulting to identical names):
...
Two routing primitives follow from the presence of a \texttt{destination} field. A \emph{directed} signal (with destination, e.g.\ \texttt{dispatch\_start/end}) transits intermediate nodes via ordinary destination routing; at its target the same four states apply (a pass declaration relays it onward as a broadcast, stripping the destination). A \emph{broadcast} signal (no destination, e.g.\ \texttt{SoS}/\texttt{EoS}) advances one declared \texttt{pass} hop at a time until the pipeline exit, reaching every subscriber in between.

The emitting side is declared symmetrically: each module lists the signals it emits (by internal name) in its class contract \texttt{EMIT\_SIGNALS}, and sends through \texttt{emit\_signal()}; the wire name is renamable via the config's \texttt{emit\_signals} mapping. Renaming emitted envelopes is what makes \emph{nested} dispatcher/receiver brackets wireable: the inner pair emits, say, \texttt{inner\_dispatch\_start/end}, which the inner receiver catches, without colliding with the outer envelope. The dispatcher's own signal semantics follow its logical mainline: its \texttt{pass\_signals} relay \emph{directed to the receiver} (transiting branch nodes, which neither see nor declare them), while \texttt{dispatch\_signals} (parallel to \texttt{dispatch\_vars}, both referencing renamed target names) direct per-branch copies; catch-renaming keeps the branch-directed name distinct so branches do not double-trigger.

Pipelines are validated statically at initialization time, and the validation is deliberately \emph{per-node}: each module declares its own contract as class attributes (\texttt{REQUIRED\_CATCH\_SIGNALS} / \texttt{REQUIRED\_INPUTS}, config-dependent variants via classmethods, plus \texttt{EMIT\_SIGNALS} for what it sends), and the validator only checks every node's config against its own module's contract --- required catches covered by catch targets (renamed wiring still satisfies the contract, since handlers match target names), required inputs covered by \texttt{input\_vars}, and node-internal reference consistency (emit renames must name real emissions; dispatcher's \texttt{dispatch\_vars}/\texttt{dispatch\_signals} must reference its own output/catch targets). Cross-node flow is intentionally \emph{not} modeled: doing so would replicate each module's routing semantics inside the validator and drift as modules evolve. Broken links between nodes instead surface at runtime through the four-state rules --- an undeclared signal is dropped with a loud per-client log entry at the first node it reaches. Any static finding rejects the pipeline (HTTP 400 with details).
...
The state machine is purely client-side; the server does not track client state. For motion generation pipelines, the client includes an SMPL-H motion player that retargets received joint parameters onto the character skeleton, playing back a frame buffer at the motion's native framerate with crossfade blending between motions.
