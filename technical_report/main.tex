...
Some modules process data spanning multiple messages (e.g., AudioCollector accumulates frames between \texttt{vad\_start} and \texttt{vad\_end}). The \texttt{SpanProcessingStep} base class provides: \texttt{start\_span(timestamp)} to begin accumulation, \texttt{end\_span()} to complete it, and \texttt{on\_span\_cancel()} to discard accumulated state on cancellation.
