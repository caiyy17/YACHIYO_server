...
In WebRTC mode, the pipeline successfully delivers frame-level synchronized output over a 45-second test session:
\begin{itemize}[nosep]
    \item 901 DataChannel messages (one 100\,ms frame group each)
    \item 2262 audio frames sent, 2259 received (48\,kHz stereo)
    \item 1357 video frames sent and received (30\,fps)
    \item ASR correctly transcribes the test input via the AudioCollector--ASR path
