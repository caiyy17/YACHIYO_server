...
The one exception requiring out-of-band delivery is cancellation, discussed in Section~\ref{sec:cancel}: cancellation is \emph{not} a signal---its semantics are deliberately order-free (invalidate everything older than $T$), which is exactly why it may bypass the in-band ordering that signals must respect.
...
The inequality is deliberately \emph{strict}, uniformly across server and client; whether the boundary is included is expressed by the issuer's choice of $T$ (nudged by an epsilon where needed---e.g.\ a barge-in cancel spares the concurrent same-instant utterance, while pipeline teardown uses an infinite stamp to void everything).
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Already emitted by the final node}: the message has left the pipeline; delivery-side handling belongs to the consuming edge---the WebRTC gateway drops stale-stamped messages when unpacking groups, and an interrupting client flushes its own playout.
\end{itemize}
