...
Each node has a dedicated \texttt{cancel\_queue} (parallel to its data queue), checked before each dequeue operation. When the client sends $\texttt{cancel}(T)$, the server broadcasts $T$ to every node's cancel queue. Each node updates its local \texttt{cancel\_timestamp}; a dequeued message \emph{addressed to the node} with timestamp $<$ \texttt{cancel\_timestamp} is silently dropped --- the gate runs where a message is consumed, while forwarding hops relay unexamined.
