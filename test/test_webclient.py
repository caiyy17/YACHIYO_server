...
WAV = "/home/cyy/Documents/Projects/YACHIYO_server/test/test_voice.wav"
FAIL = []
...
starts = [i for i, e in enumerate(events) if e == "recording_start"]
check(f"2 presses, each cancel-first {events}",
      len(starts) == 2 and all(i > 0 and events[i-1] == "cancel"
                               for i in starts))
check("page saw a reply (SoS logged)", "SoS" in page_log)

log = open(f"logs/client_{cid}.log").read()
check("server got both turns (2 vad start marks)",
