...
| 服务                  | 目录                                 | 说明                                      | 协议                                                                  |
| --------------------- | ------------------------------------ | ----------------------------------------- | --------------------------------------------------------------------- |
| ASR (SenseVoice)      | `Modules_standalone/SenseVoice/`     | SenseVoice 的 OpenAI Whisper 兼容 wrapper | [Apache 2.0](https://github.com/FunAudioLLM/SenseVoice)               |
| LLM (vLLM)            | `Modules_standalone/VLLM/`           | vLLM 原生 OpenAI API 的配置文件           | [Apache 2.0](https://github.com/vllm-project/vllm)                    |
| TTS (BertVITS2)       | `Modules_standalone/BertVITS2/`      | Bert-VITS2 的 OpenAI TTS 兼容 wrapper     | [AGPL-3.0](https://github.com/fishaudio/Bert-VITS2)                   |
...
**可以替换任何服务**为任何暴露相同 OpenAI 兼容 API 的实现（例如用 Ollama 替换 vLLM，或用 OpenAI 的 TTS API 替换 BertVITS2），只需修改配置文件。
...
客户端 (WebRTC / WebSocket)
  |
  v
server_webrtc.py (端口 18082)     <-- WebRTC 桥接（可选）
  |  WebSocket
  v
