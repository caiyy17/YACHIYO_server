...
把工作流化为线性流程的一个很大的好处是时序处理不会混乱：无论对于任何节点，先到达地信息一定会被优先处理，优先输出，不存在有任何信息可以超车（在非线性流程中，某些信息可以走更短地路径从而打乱输出顺序）。注意对于一个消息，它只能同时被一个节点处理，并没有两个节点同时处理后汇合的能力。

为了流程的便利性，我们允许节点直接向最终输出发送信号（这个是超车信号），这样可以即时把消息传回给 client。
