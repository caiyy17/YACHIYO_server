...
- **Declared interface contracts** — every signal (catch/pass/emit) and every input/output var is declared in the pipeline config as explicit one-to-one `{source, target}` entries (both fields always written; renames included); undeclared signals never drift through a node, and relayed copies travel edge by edge like data. Per-node contracts are validated statically at init, exactly both ways: catch targets == the module's required catches, emit declarations == its EMIT_SIGNALS, input targets == its declared inputs, output sources == its products. An explicit `null` on the wire side is a declared opt-out: default input / unsent output / unwired catch / suppressed emission. Control-plane **events** (e.g. `connection_start`, `playback_complete`) are declared the same way — a top-level `events` list routes them at the entry, per-node `catch_events` entries consume them — but are broadcast out of band into every node's control queue instead of traveling the data path (`cancel`/`kill` are built-in and never declared)
...
| `unity_chan_default`  | ASR → LLM → DataQuery → Dispatch → MotionRAG ∥ TTS → Receive → Pad | Conversation with RAG expression + action matching (parallel example) |
...
| `unity_chan_humanoid` | ServerVAD → ASR → LLM → DataQuery → StreamTTS → MotionChunk | Streaming conversation with audio-driven motion |
| `unity_chan_live`     | DanmakuBuffer → LLM → DataQuery → StreamTTS → MotionChunk | VTuber danmaku livestream (streaming chunks) |
